...
class PropertyService: ObservableObject {
    ...
    func updateProperty(_ updatedProperty: Property) {
        print("🟢 UPDATE PROPERTY CALLED")
        print("🟢 Looking for property ID: \(updatedProperty.id)")
        if let index = properties.firstIndex(where: { $0.id == updatedProperty.id }) {
            ...
            print("🟢 Found property at index: \(index)")
                    print("🟢 Old camera: '\(properties[index].camera)'")
                    print("🟢 New camera: '\(updatedProperty.camera)'")
                    
                    properties[index] = updatedProperty
                    saveProperties()
                    
                    print("🟢 Property updated and saved")
                    NotificationCenter.default.post(name: .propertiesDidChange, object: nil)
                } else {
